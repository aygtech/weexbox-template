 ...
 * Description: This is SplashFragment
 */
        ...
        UpdateManager.update { state, progress, error, url ->
            when (state) {
                UpdateManager.UpdateState.Unzip -> {
                    // 解压
                    (activity as SplashActivity).loadDialogHelper.showProgressWithText(activity, "解压中", progress)
                }
                UpdateManager.UpdateState.DownloadFile -> {
                    // 下载
                    if (UpdateManager.forceUpdate) {
                        (activity as SplashActivity).loadDialogHelper.showProgressWithText(activity, "下载中", progress)
                    }
                }
                UpdateManager.UpdateState.UpdateSuccess -> {
                    ...
                    router.closeFrom = 0
                    router.open(activity as SplashActivity)
                }
            }
        }
    }
